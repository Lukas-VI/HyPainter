...
internal data class CanvasDebugState(
    ...
    val pointerId: Int = -1,
    val activeStylusPointerId: Int? = null,
    ...
    val recoveredStylusPointer: Boolean = false,
    ...
    fun toLogLine(viewport: ViewportState): String {
        return "route=$route action=$action tool=$toolType pointers=$pointerCount consumed=$consumed " +
            "pointerId=$pointerId activeStylus=$activeStylusPointerId recovered=$recoveredStylusPointer " +
            "pressure=${"%.2f".format(pressure)} screen=${screenPosition.debugFormat()} canvas=${canvasPosition.debugFormat()} " +
            "samples=$strokeSamples history=$historySamples historySize=$historySize eventTime=$eventTime downTime=$downTime " +
            "pan=${viewport.pan.debugFormat()} " +
