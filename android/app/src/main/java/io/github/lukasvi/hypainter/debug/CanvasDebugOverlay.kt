...
internal fun CanvasDebugOverlay(
    ...
    Column(
        ...
        Text("Pointer: id=${state.pointerId} activeStylus=${state.activeStylusPointerId}", color = Color.White)
        Text("Consumed: ${state.consumed}", color = Color.White)
        Text("Samples: ${state.strokeSamples} history=${state.historySamples}/${state.historySize}", color = Color.White)
        Text("Time: event=${state.eventTime} down=${state.downTime} recovered=${state.recoveredStylusPointer}", color = Color.White)
